...
def emb_mean_pooling_user(emb_all, num_per_record, num_user):
    for idx in range(num_user):
        ...
        emb_user_one = emb_all[idx_low:idx_high,]
...
def save_checkpoint(output_dir, model, idx_epoch, idx_step, best_f1, best_acc, logger):
    ...
    output_dir = os.path.join(
        output_dir,
        f"epoch_{idx_epoch}-step_{idx_step}-best_f1_{best_f1:.4}-best_acc_{best_acc:.4f}.pt",
    )
...
def get_pool_emb(
    hidden_state=None, attention_mask=None, pool_strategy="cls", custom_layer=None
):
    """获取句向量"""
...
def get_scheduler_batch(optimizer, mode, lr_base, num_epoch, num_batch, **kwargs):
    ...
    if mode == "StepLR":
        ...
        list_milestones = [
            num_training_steps * rate for rate in [0.2, 0.5, 0.7, 0.8, 0.9]
        ]
        scheduler = MultiStepLR(optimizer, milestones=list_milestones, gamma=0.8)
    elif mode == "LambdaLR":
        scheduler = LambdaLR(optimizer, lr_lambda=lambda batch: 1 / (batch + 1))
        ...
        scheduler = CosineAnnealingWarmRestarts(
            optimizer, T_0=int(num_training_steps * 0.1), T_mult=3
        )
